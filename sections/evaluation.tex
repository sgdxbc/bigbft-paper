...
This is because their design disables sharding with committee sizes smaller than these bounds, meaning they can only start to shard once they reach these scales.
\lijl{Can remind that these designs also compromise security and liveness.}
...
We perform numerical analysis to project network traffic under realistic conditions.
\lijl{Can summarize these conditions. Are the results workload dependent?}
...
The network traffic associated with these activities increases linearly with the number of nodes, as does the network traffic associated with loading shards.
\lijl{Given the fact that normalized traffic converges, can we conclude that network traffic for shard loading also grows linearly?}
Therefore, our approach does not negatively impact the scalability of network communication.
