...
We conduct a numerical analysis of the total storage consumption across various blockchain designs, and the results are presented in \cref{fig:redundancy}.
...
As we discussed in \cref{sec:bg}, its storage consumption would be equivalent to full replication with $\frac{1}{3}$ faulty nodes.
\fi
...
In the contrary, the processing sharding design disables sharding with committee sizes smaller than hundreds, not even mention that it also compromises the security properties (\cref{sec:bg}).
\lijl{Can remind that these designs also compromise security and liveness.}\sgd{Done.}
...
\lijl{Can summarize these conditions. Are the results workload dependent?}\sgd{Done.}
...
\lijl{Given the fact that normalized traffic converges, can we conclude that network traffic for shard loading also grows linearly?}\sgd{I think so.}
