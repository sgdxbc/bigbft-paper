...
\paragraph{Overall system guarantees.}
Given the above properties, we now show that the overall blockchain can guarantee conventional BFT correctness.
The consensus layer ensures that the correct nodes receive the same transaction sequence.
All correct nodes will then generate the same sequence of \fetch and \bump calls.
With that, state management ensures state availability.
As such, for all transactions, at least $f+1$ correct nodes execute them (no \forward returned for \fetch and \bump calls).
Finally, state safety ensures that all correct nodes transition to consistent state from the same initial state and generate consistent transaction results.
For each transaction, a client can thus collect $f+1$ identical responses from correct nodes.
