...
As shown in \autoref{fig:approach-compare}, a blockchain is logically partitioned into three layers: a consensus layer, an execution layer, and a state management layer.
...
While executing a transaction, the execution layer invokes the state management interface to read and write world state.
...
\gd{Suggested rewording:
A \forward indicates that the replica node can safely skip all transaction execution until version $p$ without affecting the liveness of the overall BFT system.
...
\gd{Draft that replace the remaining subsection.
The node must store some metadata, e.g., the hashes of the state, to validate the retrieved state independently, or it must collect $f+1$ matching states from different nodes which cannot happen due to the first challenge.
If the node stores only one hash or the hashes for fixed number of partitions or encoded chunks of the state, the retrieved state size would be proportional to the entire state size, and transferring such large data will be highly inefficient.
So, the storage scheme must not only be scalable, but also enable the nodes to validate the retrieved state with minimal metadata and communication.
}
The straw man solution of collecting $f+1$ matching states works, but requires all state to be stored on at least $f+1$ nodes.
This again violates our storage scalability requirement.
