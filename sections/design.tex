...
\midrule
\multirow{5}{*}{Replica to storage nodes RPC}
    & \retrieve{} request   & round, key \\
    & \retrieve{} response  & round, key, value, successor key, aggregated signature \\
    & \retrieves{} request  & round, shard ID \\
    & \retrieves{} response & round, shard ID, shard data \\
    & \pushss{}             & round, shard ID, shard share data \\
\addlinespace
\multirow{1}{*}{Storage nodes peer RPC}
    & \getsc{} request      & round, shard ID \\
    & \getsc{} response     & round, shard ID, storage node ID, shard chunk data \\
\addlinespace
...
The atomic broadcast messages are sent by submitting them as client transactions to the atomic broadcast protocol, which acts as an ordered message queue for the replica and storage nodes.
...
After getting the responses, the SMS verifies the values against the signatures, updates the cache, and returns the values to the execution layer.
...
And then, SMS extracts the previous keys and values from the retrieved shards and apply the relevant updates in the update table snapshot to them.
SMS then builds a shard share and \pushss it to the storage nodes.
The update table snapshot is kept until the SMS pushes all the shard shares for round $c'$.
...
They respond to \retrieve and \retrieves requests by returning the requested values and shards from the last completed checkpoint.
...
To checkpoint a shard, the storage node collects matching \pushss messages from $f+1$ replica nodes and constructs the shard by aggregating the signature shares.
