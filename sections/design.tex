...
After receiving $2f+1$ \votec messages for round $c'$ greater than its current checkpoint round $c$, the storage node starts to checkpoint round $c'$.
To checkpoint a shard, the storage node collects matching \pushss messages from $f+1$ replica nodes to ensure the integrity of the shard.
...
It further constructs and stores the Merkle tree for the shard for producing the proofs in \retrieve responses.
