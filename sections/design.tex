...
It constructs the Merkle tree for the shard, recording the shard commitment for the later \votec message.
Then it encodes the shard into $3f+1$ chunks, records their hashes, and stores the chunk corresponding to itself.
If the storage node is one of the responsible nodes of the shard according to the current placement, it also stores a full copy of the shard and the Merkle tree for serving retrieval.
